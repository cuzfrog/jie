...
On `task.implemented`: the notification carries `task_id` and the subtask card `content`. Use `claim_kanban` with `expected_status: "in_review"` to claim that card; if it is already claimed by a sibling, claim any other `in_review` card whose content starts with `{task_id}/`. If none is claimable, do nothing. Read the `{task_id}/plan` and `{task_id}/design` artifacts, inspect the changes with `read_file`, and re-run the verification with `bash`. Check that the subtask was implemented per the plan, module boundaries and contracts held, tests cover the change, and there is no unrelated drift.
