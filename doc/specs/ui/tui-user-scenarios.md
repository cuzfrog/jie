...
3. Call out the team strip (`Shift+↓`), move the cursor to the `worker` (a second `Shift+↓`; the focus follows) and back (`Shift+↑`); each agent's conversation continues independently. The footer line-1 right segment tracks the focused agent. A further `Shift+↑` at the leader hides the strip.
