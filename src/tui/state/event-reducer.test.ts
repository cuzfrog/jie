...
describe("reduceCompacted", () => {
  ...
  test("keeps all turns and places the marker after the summarized count", () => {
  ...
  test("without any turns only the marker is recorded", () => {
    ...
    expect(compacted.nextEntrySeq).toBe(0);
  });
  ...
  test("a later compaction advances the marker and keeps all earlier turns", () => {
    ...
    expect(compacted.nextEntrySeq).toBe(4);
  });
...
describe("reduceToolCall + reduceToolResult", () => {
  ...
  test("a write_kanban tool result updates the board and completes its tool card", () => {
    let state = promptedState();
    state = reduce(state, Events.agentToolCall(TOOL_SENDER, "c1", "write_kanban", "{}"));
    const cards = [
  ...
  test("a write_kanban tool result without a matching tool call still updates the board", () => {
    const state = promptedState();
    const cards = [
      { id: "#1", content: "alpha", status: "completed" },
      { id: "#2", content: "beta", status: "in_progress" },
