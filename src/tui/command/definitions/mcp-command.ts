...
const META = { name: "mcp", description: "show MCP servers" } as const;

export class McpCommand extends PositionalSlashCommand {
  constructor() {
    super(META);
  }

  protected override executeParsed(_context: SlashContext, _parsed: Record<string, string | undefined>): ResolvedCommand {
    return { kind: "platform", slashName: "mcp", command: { name: "listMcpServers" } };
  }
}
