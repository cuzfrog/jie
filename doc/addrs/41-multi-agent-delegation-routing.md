...
1. **Claimable work** (kanban-based). Replicas coordinate through shared state. All replicas see the same trigger (via `subscribe`) and race to claim a card with `update_kanban`. Only one wins; losers receive a clear "not claimable" result and do nothing.
...
- Use `notify` + `update_kanban` for claimable work.
