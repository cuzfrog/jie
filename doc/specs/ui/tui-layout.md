...
**Info entries** interleave with turns in `seq` order (`tui-state.md`, entry sequence). The single kind today — the `/help` reprint — renders the welcome content (wordmark + tagline, the loaded team line), the keybinding hint lines, and a static command cheat-sheet (`/help /clear /exit /team /resume /rename /model /effort /login /logout`, each with a one-line description: accent name + muted description). Every line truncates to the column width. The reprint is created once per `/help` and never mutates afterward.
