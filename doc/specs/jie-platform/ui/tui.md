...
- Prompts without an explicit agent target go to `{active_team_id}.leader.prompt` — the active team's leader receives these. The TUI tracks the active team (set by `/team <id>` or startup resolution) and routes accordingly. Other teams' bodies are untouched. Published via `Events.userPrompt({ kind: "tui" }, active_team_id, prompt)`; envelope: `topic: '{active_team_id}.leader.prompt'`; `payload: { teamId: active_team_id, prompt }`; `sender: { kind: "tui" }`; `version: 1`; `timestamp` = current ISO 8601.
- Prompts targeting a specific agent (when the user is viewing that agent's tab) go to that agent's `custom.{active_team_id}.{agent_key}` topic. Published via `Events.userPrompt({ kind: "tui" }, active_team_id, prompt, target_agent_key)` — the factory's targeted branch publishes to `custom.{active_team_id}.{target_agent_key}` using the `Events.custom` envelope shape. The envelope: `topic: 'custom.{active_team_id}.{agent_key}'`; `payload: { clientTopic: '{active_team_id}.{agent_key}', payload: { prompt } }`; `sender: { kind: "tui" }`; `version: 1`; `timestamp` = current ISO 8601. The body's subscription callback formats the message as `[user]: {prompt}` because the inner `payload` of the `Events.custom` envelope has no `source` field — the same rule the leader.prompt path uses, since the format is keyed on the presence of `envelope.payload.payload.source`, not on the subject.
...
- **Read-only on platform subjects.** The TUI MUST NOT publish to `agent.stream.*`, `agent.tool.*`, or `agent.idle`. Prompt ingress (`{active_team_id}.leader.prompt` and per-agent `custom.{active_team_id}.{agent_key}` topics, both via `Events.userPrompt`) is the TUI's sole write path.
