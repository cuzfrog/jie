...
**Effort/model hot-swap.** `user.effort.update` sets `thinkingLevel` (`max`→`xhigh`, else passthrough) and republishes `agent.model.assigned` when a model is assigned; pinned souls ignore the broadcast, mirroring `user.model.update` behavior. `user.model.update` hot-swaps the model only for bodies whose `AgentBodyParams.modelPinned` is `false`; `modelPinned` is resolved at team load (concrete or mapped-alias souls pin, unset or unmapped-alias souls follow settings) and stays immutable for the body. An unresolvable reference is a silent no-op. Both carry `contextWindow` on every publish so observers track the live model.
