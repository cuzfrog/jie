...
1. **User prompt** — prefixed with `› ` in `userMessageIcon` color (cyan); continuation lines indent 2 columns. Rendered **verbatim** (no markdown interpretation). A prompt beginning with `/skill:` replaces that prefix with a `⚡ ` icon in the same color, marking a user-prompted skill invocation.
