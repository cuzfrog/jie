...
ctrl+o tool output · ctrl+k kanban · ← team panel (cursor at start)
...
**Info entries** interleave with turns in `seq` order (`tui-state.md`, entry sequence). The single kind today — the `/help` reprint — renders `helpLines` (`components/welcome-banner.ts`) — the Commands section and the Shortcuts section with the keybinding hint lines, without the splash's mark/identity header and without any team roster (the panel shows it, `tui-team-panel.md`). Both sections are static, so the reprint never mutates. Every line truncates to the column width.
