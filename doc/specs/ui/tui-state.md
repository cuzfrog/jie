...
- `Actions.switchCycleAgent(direction: 1 | -1)` — the team-strip cursor rule over `state.teamCursorAgentId` (`tui-team-panel.md`). Hidden (`!state.teamPanelVisible`): show the strip with the cursor seeded from `state.focusedAgentId`, no move; when the focus is null, direction `1` lands on the first agent, `-1` on the last. Shown: moves the cursor without touching `focusedAgentId` — direction `1` down, wrapping last → first; direction `-1` up, except at the first agent, where it hides the strip and clears the cursor. Navigation order is `TuiState.rosterOrder` (leader first, then map insertion order) — the same order the strip displays. No-op when the agent map is empty; a stale cursor recovers to the first agent on the next press.
- `Actions.commitTeamCursor()` — set `focusedAgentId` to `teamCursorAgentId`; no-op unless the strip is shown and the cursor is set and differs from the focus; a stale cursor is cleared without moving focus. Wired to `Enter` in the view's global input listener, consumed only under that same condition so editor submit is untouched otherwise (`tui-team-panel.md`).
...
- `Actions.clearTuiState()` — clear `agents`, `leaderAgentId`, `focusedAgentId`, `teamCursorAgentId`, `interruptedAgentId`, `infoEntries`, and `errorBanner` / `transientMessage`, and reset `nextEntrySeq` to 0. `teamPanelVisible` survives like the other view toggles. Memory rows on disk are untouched. Used by the `/clear` slash command.
...
The chat column is a single chronological stream of two entry kinds — conversation turns and info entries (today only the `/help` reprint). Every entry carries a `seq`: `MessageTurn.seq` and `InfoEntry.seq` are assigned from one monotonic `state.nextEntrySeq` counter — by `user.prompt` and turn-creating `agent.turn.start` on the event side, by resume hydration numbering turns sequentially, and by `Actions.showHelp` on the UI side — advancing on each assignment, so `seq` order across both kinds is creation order. A `turn.start` reusing an unpopulated prompt turn consumes no number. Team switch (not same-team re-application) and `clearTuiState` reset the counter to 0 and clear `infoEntries`, because the chat view resets with the team. Chat sync merges the focused agent's turns with `state.infoEntries` ordered by `seq` and matches its live components by kind + `seq`, so unchanged entries update in place and a kind mismatch rebuilds the tail.
