...
const dynamicHint = { format: vi.fn(() => "HINT") };
...
describe("Footer", () => {
  ...
  test("line two shows the dynamic hint between the context metrics and the model", () => {
    stateStore.getState.mockReturnValue(makeTuiState({ ...seededStateWithModel(), editorCursorAtStart: false }));
    const plain = stripAnsi(new Footer(stateStore, tokenUsage, modelSegment, queueIndicator, compactingIndicator, dynamicHint).render(80)[1]);
    expect(plain).toContain("HINT");
    expect(plain.indexOf("HINT")).toBeLessThan(plain.indexOf("(anthropic)"));
  });

  test("line two keeps the dynamic hint before a team is loaded", () => {
  ...
  test("every line fits the given width", () => {
    stateStore.getState.mockReturnValue(seededState(true));
    const lines = new Footer(stateStore, tokenUsage, modelSegment, queueIndicator, compactingIndicator, dynamicHint).render(60);
  ...
  test("line two shows a Compacting… indicator when the focused agent is compacting", () => {
    ...
    const lines = new Footer(stateStore, tokenUsage, modelSegment, queueIndicator, compacting, dynamicHint).render(80);
...
describe("Footer.update", () => {
  ...
  test("reports dirty when the open kanban count changes", () => {
    const base = seededState(false);
    const withCards = makeTuiState({ ...base, kanban: { ...base.kanban, board: [{ id: "c1", content: "task", status: "pending", todos: [] }] } });
    stateStore.getState.mockReturnValue(withCards);
    const footer = new Footer(stateStore, tokenUsage, modelSegment, queueIndicator, compactingIndicator, dynamicHint);
    footer.update();
    stateStore.getState.mockReturnValue(makeTuiState({ ...base, kanban: { ...base.kanban, board: [{ id: "c1", content: "task", status: "pending", todos: [] }, { id: "c2", content: "task2", status: "in_progress", todos: [] }] } }));
    expect(footer.update()).toBe(true);
  });
