...
- While the panel is shown, `↑`/`↓`/`←`/`→` move the cursor card: up/down walk the current column, left/right jump to the nearest non-empty column (rows clamp to the target column's length). A cursor absent or stale recovers to the first card; an empty board renders no cursor.
...
- While editing, all keys route to the editor — cursor movement, expand/collapse, and the panel toggle's `Ctrl+K` close are suspended (closing the panel also cancels the edit).

## State

`packages/jie-tui/state/ui-reducer.ts` carries the panel's five fields (`tui-state.md`): `kanbanPanelVisible: boolean` (initial `false`), `kanbanBoard: ReadonlyArray<KanbanCard>` (initial `[]`), `kanbanCursor: string | null` (a card id, or `null` on an empty board), `kanbanExpanded: boolean`, and `kanbanEdit: string | null` (the card id being edited, or `null`). The panel re-reads state on every render and needs no subscription of its own: every dispatched action already triggers a full re-render through the chat sync (`tui-layout.md`). While the panel is shown the footer drops its second line, as with the team panel (`tui-layout.md`, Footer).