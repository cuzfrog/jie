...
The platform's `JiePlatform` surface for v0.2 is `{ bus, teamId, bodies, loadTeam, stop }`. `loadTeam` is the TUI's only new caller; the v1 CLI's `-p` mode does not adopt it.
