...
Any agent that is listed in a `team.{team_id}.loaded` event for the active team, OR publishes `agent.stream.chunk`, `agent.tool.call`, `agent.tool.result`, `agent.turn.start`, or `agent.idle` for the active team, becomes visible. The `team.{team_id}.loaded` event (per ADR 22) is the **anchor for "agent is alive"** — the TUI subscribes to it for each loaded team and populates the agents-panel from its `agents` array as soon as it arrives, before any activity event. The activity events refine the "busy / idle" status.
...
- The TUI never encounters a "leader offline" state — if the leader stops, the process has crashed. (The TUI's "agent is alive" check is the presence of the leader in a recent `team.{team_id}.loaded` event; absence of `team.loaded` for an agent is the "I have not seen this team" case, which can also arise from a fresh process that has not yet received the event, or a team that was never loaded in this process.)
...
5. Subscribe to all platform subjects (`agent.stream.chunk`, `agent.stream.end`, `agent.tool.call`, `agent.tool.result`, `agent.turn.start`, `agent.idle`, `agent.queue.update`) and `team.{team_id}.loaded` for any team that loads, plus the active team's `team.{active_team_id}.agent.{leader_agent_key}.prompt` (using the leader's `agent_key` from the active team's `team.loaded` event) and any `custom.{active_team_id}.{topic}` from the team's blueprint.
6. Build the initial state from the `roles` bootstrap list and from the next `team.loaded` event.
...
The TUI's subscription lifecycle (per "Model and Team Hot-Swap" above) ships in v0.2. Per-team subscriptions (the active team's `team.{active_team_id}.agent.{leader_agent_key}.prompt` and any blueprint-defined `custom.{team_id}.{topic}`) come and go with the active team; per-process subscriptions (`agent.stream.chunk`, `agent.tool.*`, `agent.turn.start`, `agent.idle`, `agent.queue.update`, `team.{team_id}.loaded` for any team) stay for the TUI's lifetime.
