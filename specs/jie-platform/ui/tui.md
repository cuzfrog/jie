...
The TUI publishes user prompts to the EventBus. This is the TUI's sole write path. **The wire format is the `AgentEvent` envelope** (per `03-event-system.md` "Event Envelope") — there is no shorthand or partial-publish path. The TUI fills every envelope field; subscribers receive the full envelope as the bus's `payload` parameter. The full wire-format contract is in `02-protocol-stack.md` "Prompt Ingress".

- Prompts without an explicit agent target go to `{active_team_id}.leader.prompt` — the active team's leader receives these. The TUI tracks the active team (set by `/team <id>` or startup resolution) and routes accordingly. Other teams' bodies are untouched. The envelope: `event_type: 'leader.prompt'`; `payload: { prompt }` per `PlatformEventPayload`; `team_id` = active team's id; `agent_role` and `agent_key` = the **leader's** role and `agent_key` (per the `agent_role`/`agent_key` convention in `02-protocol-stack.md` "Prompt Ingress" — the TUI resolves the target's role and `agent_key` from the team blueprint before publishing, so the envelope matches the leader's own envelope on receipt); `version: 1`; `timestamp` = current ISO 8601.
