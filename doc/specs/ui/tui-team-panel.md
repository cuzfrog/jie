...
- `←` hidden, editor cursor at the buffer start (`state.editorCursorAtStart`), kanban view not `"panel"`: show the panel with the cursor (`state.teamCursorAgentId`) on the focused agent (the first agent when the focus is null); showing it hides the kanban panel view when it is shown (the list view survives). `←` shown: hide the panel and clear the cursor. A `←` anywhere else in the buffer stays with the editor; the footer's help-info hint swaps to the shortcut hint while the shortcut is activated (`tui-layout.md`, Footer).
