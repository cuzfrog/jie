...
3. Instantiate and start `AgentBody` for each role. The handle publishes `team.{team_id}.loaded` for the startup team once all bodies' `start()` returns (per ADR 22).
...
const loadedAgentKeys = team.loaded.agents.map(a => a.agent_key);
