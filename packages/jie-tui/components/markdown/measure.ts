...
import { tableColumnWidths, tableRowText, tableRule } from "./table";
import { tokenize, runsText, type MarkdownBlock } from "./tokenize";
...
function measureTable(block: Extract<MarkdownBlock, { kind: "table" }>, width: number): number {
  ...
  let rows = rowsFor(tableRowText(block.headerRuns, widths, block.aligns), width);
  rows += rowsFor(tableRule(widths), width);
  for (const rowRuns of block.rowRuns) {
    rows += rowsFor(tableRowText(rowRuns, widths, block.aligns), width);
  }
  return rows;
}
