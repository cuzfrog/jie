...
3. Prompt the `manager`: `Update your kanban board with one in-progress card`. The manager calls `kanban_write` (implicitly available as a utility tool) and the board lands in the manager's `cards`. `Ctrl+K` shows the kanban panel; `←` then shows the team panel in its place — the two share the bottom slot and never render together; `Ctrl+K` again shows the kanban panel back and clears the team cursor; a final `Ctrl+K` hides it (`tui-kanban-panel.md`).
