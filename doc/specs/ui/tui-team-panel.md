...
- `Ctrl+↓` hidden: show the strip with the cursor (`state.teamCursorAgentId`) on the focused agent (the first agent when the focus is null). `Ctrl+↓` shown: hide the strip and clear the cursor.
- Shown + `↓` / `↑`: move the cursor without switching the focused agent, cycling both ways (last → first and first → last). The editor's prompt-history walk yields while the strip is shown; while the autocomplete popup is open, plain arrows navigate the popup instead (`tui-shortcuts.md`).
...
- No agents loaded: `Ctrl+↓` is a no-op.
...
`state.teamPanelVisible: boolean` (initial `false`) and `state.teamCursorAgentId: AgentId | null` (initial `null`), set by the `toggleTeamPanel` action, the `switchCycleAgent` cursor rule, and the `commitTeamCursor` action (the ui-reducer). `clearTuiState` resets the cursor but not the visibility — the same class as `thinkingExpanded` / `toolCardsExpanded`. The view's global input listener consumes `Enter` before the editor only while `teamPanelVisible && teamCursorAgentId !== null && teamCursorAgentId !== focusedAgentId`, and routes plain `↑`/`↓` to the cursor rule only while `teamPanelVisible` and no autocomplete popup is showing (`components/view.ts`), so a plain submit and the editor's history walk are untouched otherwise. The strip re-reads state on every render and needs no subscription of its own: every dispatched action already triggers a full re-render through the chat sync (`tui-layout.md`).
