...
**v1 deferral.** `MemoryManager.compact()` is storage-ready and unit-tested, but no v1 caller exists: the body's `transformContext` wrapper is the identity function. Day 2 lands the diffing wrapper and the trigger wiring together — keeping storage and trigger in lockstep avoids a partial implementation that would silently lose compaction events.
