...
describe("ChatPane scroll confinement", () => {
  ...
  test("scrolling onto an inter-turn separator row paints it at the viewport top", () => {
    const stateStore = createStateStore();
    stateStore.dispatch(Actions.receiveEvent(Events.teamLoaded({ kind: "system" }, {
      id: "demo",
      leaderKey: "general-1",
    ...
    const sender = { kind: "agent", teamId: "demo", agentKey: "general-1" } as const;
    const twoLine = (first: string, second: string): string => `${first}  \n${second}`;
    stateStore.dispatch(Actions.receiveEvent(Events.userPrompt({ kind: "user" }, "demo", "prompt-0", "general-1")));
    stateStore.dispatch(Actions.receiveEvent(Events.agentTurnStart(sender)));
    stateStore.dispatch(Actions.receiveEvent(Events.agentStreamChunk(sender, 1, 0, "text", twoLine("alpha", "beta"))));
    stateStore.dispatch(Actions.receiveEvent(Events.agentIdle(sender, "stop")));
    stateStore.dispatch(Actions.receiveEvent(Events.userPrompt({ kind: "user" }, "demo", "prompt-1", "general-1")));
    stateStore.dispatch(Actions.receiveEvent(Events.agentTurnStart(sender)));
    stateStore.dispatch(Actions.receiveEvent(Events.agentStreamChunk(sender, 2, 0, "text", twoLine("gamma", "delta"))));
    stateStore.dispatch(Actions.receiveEvent(Events.agentIdle(sender, "stop")));
    stateStore.dispatch(Actions.scrollChat("demo:general-1", 3));
    const state = stateStore.getState();
    const ctx = makeContextValue({ stateStore, state });
    const { lastFrame, unmount } = render(
      <TuiContext.Provider value={ctx}>
        <ChatPane width={80} height={4} />
      </TuiContext.Provider>,
    );
    const frame = (lastFrame() ?? "").replace(/\[\d+m/g, "");
    const lines = frame.split("\n");
    expect(lines[0]?.trim()).toBe("");
    expect(lines[1]).toContain("prompt-1");
    unmount();
  });
});