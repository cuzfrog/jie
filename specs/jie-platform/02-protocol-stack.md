...
| Status tracking | **Artifact store** (SQLite-backed) | Append-only status records per work unit. Latest row is canonical state. See `04-artifact-store.md`. |
| Agent → Tool | Direct function call | Tools are plain typed functions. `notify` is the LLM's only way to publish an event; the body mediates the publish. See `05-agent-model.md`. |
...
| External integrations | **MCP** (any configured server) | Configured in `.jie/mcp.yaml`. Platform is agnostic of specific servers. |
