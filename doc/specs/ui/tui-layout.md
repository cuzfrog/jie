...
The editor is pi-tui's `Editor` subclass (`components/editor/jie-editor.ts`), full width, focused at startup. Its autocomplete provider is jie's (`autocomplete/jie-autocomplete.ts`): slash commands, `/team`, `/resume`, `/model`, `/model-filter`, `/login`, `/logout`, and `/effort` argument completion ("Selection via editor autocomplete" above), and `@`-file mentions. Top + bottom borders in `borderMuted`; when the buffer parses as a bash command (`!cmd` / `!!cmd`), both borders flip to `warning` color for the duration. Grows by one row per typed `\n`; never reserves a static row budget. Key handling is pi's verbatim plus three jie keys (`tui-shortcuts.md`). While the autocomplete popup is open, the highlighted suggestion's tail renders as `dim` ghost text right after the cursor, tracking the popup selection (`Up`/`Down`); it appears only when the selected value extends the typed prefix, and consumes trailing padding so the row width is unchanged. For slash commands the ghost also carries the command's argument hint — the `<…>`/`[…]` head of its popup description — e.g. `/mo` ghosts `del <provider/modelId>`. The editor's target is `state.focusedAgentId` (fallback: `state.leaderAgentId`); `onChange` syncs `state.editorText` and clears banners on the first keystroke after an error; `onSubmit` appends to prompt history (persisted as one JSON line per entry in `<jie home>/prompt-history.jsonl`, seeded back into the `Up`/`Down` walk at the next startup) and dispatches `submitEditorText`.
