...
| **Soul** | Declares an agent's behavioral profile: its model, system prompt, tool list, topic subscriptions, and skill specs. Immutable at runtime; carries no mutable state. |
| **Body** | The concrete `AgentBody` instance. Holds a soul plus its event-manager, artifact-store, and transcript-store references and its `session_id`; runs the event loop and publishes events on behalf of the soul. Identified by `agent_key = {role}-{N}`. |
...
| **TeamInfo** | The roster published in `system.team.loaded` and returned by the `team` command: `{ id, leaderKey, agents: AgentInfo[] }` with `AgentInfo = { teamId, role, agentKey, isLeader, tools, subscribe, skills, model }`. The boot "this agent exists" signal; the TUI's agent-discovery primitive. |
...
| **Tool** | A typed function with a JSON schema, available to an agent's LLM. Built-ins: `notify`, `bash`, `read_file`, `write_file`, `edit`, `write_kanban`, `web_search`, `web_fetch`, `write_artifact`, `read_artifact`, `memory_search`. MCP-backed tools are the planned extension (not implemented today, ADR 4). |
...
| **Storage** | The persistence abstraction: `exec`, `query`, `transaction`, `close`. `SqliteStorage` (bun:sqlite) is the only implementation; domain code never imports `bun:sqlite`. One instance per process backs the domain stores. See `04-storage.md`. |
...
| **TranscriptStore** | The domain interface for an agent's conversation transcript (`persist`, `compact`, `restore`, `hasSession`) on top of `Storage`. `memory_turns` is conversation history, not work products; shares the one DB file. See `08-transcript.md`. |
| **Compaction** | Summarizing an agent's LLM context window. Driven by the body's `Compactor` between runs on top of pi-agent-core's compaction toolkit (`06-agent-model.md` "Compaction"); the summarized prefix is flagged `compacted=1` in `memory_turns` and replaced by a `compactionSummary` row; not visible on the event bus. |
| **Memory** | Team-scoped long-term memory: atoms (`fact` / `decision` / `method` / `instruction`) extracted from compacted conversation prefixes, stored in `memory_atoms`, searched via FTS5. Recalled as a budgeted system-prompt block at body restore; searched via the `memory_search` tool. Derived from TencentDB-Agent-Memory's L1 layer (MIT; ADR 34). See `11-memory.md`. |
| **LlmService** | The platform's one-shot model-call path outside agent sessions: model in, text out, credentials resolved internally. Consumers: compaction, memory extraction. See `07-llm-service.md`. |
...
| **Built-in Default-Solo Team** | The platform's last-resort fallback: a single `general` agent. Shipped as `team/default-solo/TEAM.md` + `general.md` (the same `.md` format as user teams), embedded via `import` attributes. See `default-solo-team.md`. |
