# ADR 15: v1 MVP Scope — TUI Stub, jie-cli Subset, jie-team/code-lens Out

## Status

Accepted. Defines what is and is not in the v1 MVP of jie-platform.
...
- `jie-team` — a manifest package (per ADR 11)
...
3. **jie-team and code-lens**: Out of v1 scope per user direction — what does that mean for the repo layout and the platform's dependencies?

ADR 11 already settles the package-boundary question for jie-team. What remains is: how do we cut v1 down to ship a runnable platform without dragging in TUI / CLI / team / code-lens work that has not been signed off on.
...
- `packages/jie-team/package.json` exists as a placeholder with `name: "@cuzfrog/jie-team"`, `files: ["teams/"]` per ADR 11, and an empty `teams/` directory. No install hook, no `postinstall`. The package is not a dependency of `jie-platform` or `jie-cli`; the umbrella `package.json` may or may not list it as a workspace.
...
- The dev team manifests — these live in `jie-team`, which is out of scope. The platform ships only the built-in minimal team (1 role, 1 tool list). User teams can be added by hand to `.jie/teams/<id>/`.
...
- **The platform's runtime is the deliverable.** The TUI, jie-team, and code-lens are downstream consumers; their absence does not prevent the platform from being runnable end-to-end (via `-p` mode + the built-in minimal team).
...
- The platform's `startJie` resolves the startup team (the `JiePlatformDeps.teamRegistry.loadTeam(...)` call inside `createJiePlatform`) via the standard-path resolution per `10-configuration.md`. With no user teams installed and no dev team shipped, `loadMinimalTeam()` is the only team the v1 MVP can resolve to (after the user runs `jie login` and `jie model`). The handle's public surface is `{ bus, stop }` only; the multi-team design is Day 2+ (ADR 19).
- `00-user-scenarios.md` scenarios that require the TUI are documented as deferred. The `-p` mode scenarios and `jie login` / `jie model` setup flow are the v1 test surface. The TUI scenarios re-enter the test plan when `packages/jie-tui/` is implemented.
