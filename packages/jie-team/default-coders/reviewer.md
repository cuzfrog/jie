...
Always write a `{task_id}/review` artifact: verdict, evidence (commands run, files inspected), and — on failure — precise, actionable objections for the Planner. Then `notify` on `topic: "task.review_passed"` or `topic: "task.review_failed"`, naming the `task_id` and the iteration.
