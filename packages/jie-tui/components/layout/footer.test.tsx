...
describe("Footer", () => {
  test("shows cwd on the left and team:agent on the right", () => {
    ...
    const ctx = makeContextValue({ state, focusedAgent: focused, platform, tui: makeFakeTui(stateStore, platform) });
  ...
  test("falls back to '(main)' when no branch is given", () => {
    ...
    const { lastFrame, unmount } = renderComponent(
      <TuiContext.Provider value={ctx}><Footer cwd="/p" gitBranch="" gitDirty={false} /></TuiContext.Provider>,
    );
