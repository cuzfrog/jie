...
The state shape, action union, and reducer implementations live in `src/tui/state/` (`state.ts`, `actions.ts`, `event-reducer.ts`, `ui-reducer.ts`, `reducer.ts`). This spec is the contract they satisfy; code shapes are not duplicated here.

## Reducer purity model

The reducer is a pure function `(state, action) → state`. `StateStoreImpl` (`src/tui/state/state-store.ts`, constructed by the `bootTui` container) wraps the pure reducer in a `dispatch` function on the `StateStore`; re-render is driven through the store's subscriber line. **The clock is not read inside the reducer** — spinner frames and transient-message aging live entirely on the render side, and thinking durations arrive pre-measured (`thinking_durations` on `agent.stream.end`, timed at the platform's streaming edge); the reducer only stamps them. UI actions like `Actions.setTransientMessage(text)` carry no timestamp; the renderer records `Date.now()` when it dispatches.
...
The kanban board is **team-scoped, not per-agent**: `state.kanbanBoard: ReadonlyArray<KanbanCard>` (`KanbanCard` and the tool-result `details` union come from `src/platform`) holds the loaded team's cards, seeded from `teamInfo.kanbanCards` on load and replaced wholesale by the platform's kanban command results (`tui-kanban-panel.md`). The `agent.tool.result` `write_kanban` payload completes a tool card like any other — the board is a separate data channel, updated from the result's `{ kind: "kanban", cards }` details.
...
The reducer takes `Action = ReceiveEvent | SwitchTeam | SetSessionName | SetInstalledTeams | ToggleThinking | ToggleToolCards | SwitchCycleAgent | ToggleTeamPanel | CycleKanbanView | CommitTeamCursor | ClearTuiState | SetTransientMessage | ClearTransientMessage | SetErrorMessage | ClearErrorMessage | ClearBanners | RequestQuit | RequestRender | SetEditorText | SetEditorCursorAtStart | SubmitEditorText | RequestInterrupt | RequestDequeue | RequestRequeue | SetEnvironment | ShowHelp | SetKanbanBoard | MoveKanbanCursor | ToggleKanbanExpand | MoveKanbanEditField | CommitKanbanEdit | CancelKanbanEdit | SaveKanbanEdit` (defined in `src/tui/state/actions.ts`). Bus envelope types are **not** the action type — `tui.ts` wraps every bus envelope in `Actions.receiveEvent(envelope)` before dispatch. UI-local events (switch team, cycle, team panel, kanban view, transient, error, clear, quit, render, editor text, submit, interrupt, dequeue, requeue, environment, show help) are dispatched directly.
...
- `Actions.toggleTeamPanel()` — flip `state.teamPanelVisible` (`tui-team-panel.md`). Showing seeds `teamCursorAgentId` from the existing cursor, else `focusedAgentId`, else the first roster agent, and when the kanban view is `"panel"` hides it, clearing `kanbanEdit` and `kanbanExpanded` (the `"list"` view coexists with the team panel); hiding clears the cursor. No-op when the roster is empty (before a team is loaded). Wired to `←` at the editor start in the view's global input listener.
- `Actions.cycleKanbanView()` — cycle `state.kanbanView` hidden → list → panel → hidden (`tui-kanban-panel.md`). Entering `"panel"` hides the team panel and clears its cursor — the two panels share their bottom slot and never render together — and clamps `kanbanCursor` into the current board; leaving `"panel"` clears `kanbanEdit` and `kanbanExpanded`; entering `"list"` has no side effect (the list view coexists with the team panel). No-op when no agent is focused (before a team is loaded). Wired to `Ctrl+K` in the view's global input listener and to `/kanban` with no args in the command handler.
- `Actions.setKanbanBoard(board)` — replace `state.kanbanBoard` and clamp the cursor into it (`kanbanReducer`, `kanban-reducer.ts`). Fired with the platform's returned board from every kanban command (`/kanban add|remove|complete`, the editor's save path) so the panel and the task list always mirror the platform.
- `Actions.moveKanbanCursor(direction)` — move `state.kanbanCursor` (a card id) across the board: up/down walk the current status column, left/right jump to the nearest non-empty column, clamping to the target length (`kanbanReducer`, `kanban-reducer.ts`). Wired to the arrow keys while the panel is shown.
- `Actions.toggleKanbanExpand()` — flip `state.kanbanExpanded` (the panel's board ↔ card-detail view) and reset `state.kanbanEditField` to `"content"`. Wired to `Tab` while the panel is shown.
- `Actions.moveKanbanEditField(direction)` — toggle `state.kanbanEditField` between `"content"` and `"description"` (`"up"` to content, `"down"` to description). Wired to `↑`/`↓` while the panel is expanded.
- `Actions.commitKanbanEdit(cardId, field)` — set `state.kanbanEdit = cardId` and `state.kanbanEditField = field`; the editor takes over for the selected field (`tui-layout.md`, Editor). Wired to `Ctrl+E` while the panel view is shown and a card is focused (`Enter` stays the prompt submit; `tui-kanban-panel.md`).
...
- `Actions.saveKanbanEdit(cardId, text, field)` — set `kanbanEdit = null`; the TUI host observes the action and runs the platform `kanbanEdit` command with the field and text, dispatching `setKanbanBoard` with the returned board (`tui.ts`). Wired to `Enter`/`Ctrl+S` while editing.
...
- `Actions.clearTuiState()` — clear `agents`, `sessionName`, `leaderAgentId`, `focusedAgentId`, `teamCursorAgentId`, `interruptedAgentId`, and `errorBanner` / `transientMessage`, reset `nextEntrySeq` to 0, and close the help panel by setting `helpPanelVisible` to `false`. `teamPanelVisible` and `kanbanView` survive like the other view toggles, and `editorCursorAtStart` survives because `/clear` does not move the editor cursor. Memory rows on disk are untouched. Used by the `/clear` slash command.
- `Actions.showHelp()` — toggle `state.helpPanelVisible`. Opening the panel hides an open team panel or kanban panel view; closing it leaves the other panels unchanged. The help panel renders the full Commands and Shortcuts sections from `helpLines` (`components/welcome-banner.ts`) in a boxed panel below the footer. Used by the `/help` slash command.
...
The chat column is a single chronological stream of two entry kinds — conversation turns and the compaction marker. Both carry a `seq`: `MessageTurn.seq` and the marker's `seq` are assigned from one monotonic `state.nextEntrySeq` counter — by turn-creating `agent.turn.start` and by `agent.compacted` (marker plus renumbered survivors) on the event side, and by resume hydration numbering the marker and turns sequentially — advancing on each assignment, so `seq` order across the stream is creation order. A `turn.start` adopting an unpopulated empty-prompt turn consumes no number. Team switch (not same-team re-application) and `clearTuiState` reset the counter to 0 because the chat view resets with the team. Chat sync merges the focused agent's turns and compaction marker ordered by `seq` and matches its live components by kind + `seq`, so unchanged entries update in place and a kind or seq mismatch rebuilds the tail.

## Out of scope for v0.2

- **Per-block / per-card `expanded` state.** Expansion is a render concern, not a state concern. The reducer only carries the all-or-nothing view toggles `thinkingExpanded` / `toolCardsExpanded` / `teamPanelVisible` / `helpPanelVisible` / `kanbanView` (`Ctrl+T` / `Ctrl+O` / the `←` toggle / `/help` / `Ctrl+K`) and the all-or-nothing kanban `kanbanExpanded` (board ↔ focused-card detail); the components read them. The view toggles and the board are preserved across `clearTuiState` (see `tui-team-panel.md`, `tui-kanban-panel.md`).
