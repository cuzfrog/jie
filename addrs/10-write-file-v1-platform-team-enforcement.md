...
ADR 9 made `read_file` a v1 platform tool. The natural sibling, `write_file`, was previously deferred to Day 2 on the rationale that its behavior is "entangled" with the **frozen-rule enforcement**: the writer would need to parse the file, extract public symbols, canonicalize, and compare against the module descriptor before writing.
...
`write_file` is a v1 built-in platform tool that enforces only **workspace-root containment** (sibling of `read_file` per ADR 9). It does **not** enforce module boundaries, frozen rules, or any team-defined constraint.
...
| Team descriptor / frozen rule | "Inside the allowed module boundary" | At the role's system prompt or via a wrapper tool the team defines | jie-team backlog (Day 2) |
