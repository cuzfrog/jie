...
- **`notify(topic, prompt)`** (all agents) publishes to `{topic}` on the EventBus with prompt content and source identity. The publishing agent's `AgentBody` filters its own receipts by `payload.source === agent_key` (see `06-agent-model.md` "Built-in Tool: `notify`" step 3); the bus itself does not filter. `notify` does not end the LLM's turn.
