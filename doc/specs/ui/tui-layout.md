...
4. **Tool cards** (`tool-call` + matching `tool-result`) — expanded shows input, output (the reducer unwraps `{content, details, terminate}` envelopes to the string content), and a diff view when the result carries one. Collapsed, successful diff-less results fold into the **work summary**, while running calls, error results, and diff-carrying results keep one header line each: the `✓` glyph in `success` (`✗` in `error` for failures), then name and formatted duration in `toolTitle` (`error` for failures). `Ctrl+O` expands / collapses all (`state.toolCardsExpanded`).
