...
const teamTable = new TeamTable(tokenUsage);
...
describe("TeamPanel", () => {
  ...
  test("renders nothing before a team is loaded", () => {
    expect(new TeamPanel(stateStore, teamTable).render(80)).toEqual([]);
  });
  ...
  test("draws a thin full box around dim column titles above one row per agent", () => {
    ...
    const lines = new TeamPanel(stateStore, teamTable).render(120);
  ...
  test("identifies the leader with a dim label instead of a mark, and drops the role from the identity", () => {
    stateStore.getState.mockReturnValue(teamState({ focusedAgentId: LEADER_ID }));
    const text = new TeamPanel(stateStore, teamTable).render(120);
  ...
  test("without a cursor the pointer follows the focused agent", () => {
    ...
    const text = new TeamPanel(stateStore, teamTable).render(120).map(stripAnsi);
  ...
  test("shows context right after the key, then tools, subscriptions, and the full model segment", () => {
    ...
    const row = stripAnsi(new TeamPanel(stateStore, teamTable).render(160)[2]);
  ...
  test("right-aligns the model column one cell inside the box and pads every line to the full width", () => {
    ...
    const lines = new TeamPanel(stateStore, teamTable).render(160);
  ...
  test("left-aligns the middle columns across rows of differing widths", () => {
    ...
    const text = new TeamPanel(stateStore, teamTable).render(160).map(stripAnsi);
  ...
  test("drops the middle columns right to left when the width does not fit them all", () => {
    ...
    const panel = new TeamPanel(stateStore, teamTable);
  ...
  test("keeps the identity and a truncated model when even those exceed the width", () => {
    ...
    const row = stripAnsi(new TeamPanel(stateStore, teamTable).render(40)[2]);
  ...
  test("shows a spinner frame for a busy agent", () => {
    ...
    const row = new TeamPanel(stateStore, teamTable).render(80)[3];
  ...
  test("marks an idle agent whose last stop was an error", () => {
    ...
    expect(new TeamPanel(stateStore, teamTable).render(80)[3]).toContain(style("error")("✗"));
  });
  ...
  test("tags the queue depth when prompts are queued", () => {
    ...
    expect(stripAnsi(new TeamPanel(stateStore, teamTable).render(120)[3])).toContain("coder-1 q2");
  });

  test("truncates every line to the available width", () => {
    stateStore.getState.mockReturnValue(teamState({ focusedAgentId: LEADER_ID }));
    for (const width of [1, 6, 12, 80]) {
      for (const line of new TeamPanel(stateStore, teamTable).render(width)) {
