...
Decide which modules the task touches, what their public contracts become (the no-new-exports boundary: public signatures change only through your `CONTEXT.md` update), and the dependency direction. Update the affected `CONTEXT.md` files with `write_file`. Write the design — modules, contracts, constraints — to the artifact `{task_id}/design`, then `notify` on `topic: "task.designed"` with the `task_id` parameter and a prompt naming it.
