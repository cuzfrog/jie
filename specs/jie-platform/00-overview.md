...
| **Leader Agent** | A team designates one agent role as its leader — the sole external-facing entry point. The leader auto-subscribes to `leader.prompt` (team's view; the bus subject is `{team_id}.leader.prompt` per ADR 21) and handles user prompt ingress. Has no special tools — uses `notify` like any other agent. |
