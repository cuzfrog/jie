...
3. Instantiate and start `AgentBody` for each role. The handle publishes `{team_id}.team.loaded` for the startup team once all bodies' `start()` returns (per ADR 22).
...
6. Publish the `AgentEvent` envelope to `{startup_team_id}.leader.prompt` (the bus subject is team-scoped per `03-event-system.md` and ADR 19). The envelope's `event_type` is `leader.prompt`; `payload` is `{ prompt: "<instruction>" }` per `PlatformEventPayload`; `team_id` is the startup team's id; `agent_role` and `agent_key` are the **leader's** role and `agent_key` (so the envelope matches the leader's own envelope on receipt); `version` is `1`; `timestamp` is the current ISO 8601 string. The CLI fills every envelope field — there is no shorthand or partial-publish path. The full wire-format contract is in `03-event-system.md` "Event Envelope" and `02-protocol-stack.md` "Prompt Ingress".
