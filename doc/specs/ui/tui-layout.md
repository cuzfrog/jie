...
- **`/model-filter `** — the two actions `add` and `remove`; after `remove `, the stored patterns (`getModelFilters`) are offered as candidates whose committed value includes the action (`remove <pattern>`); `add` stays free-form.
