...
describe("Scenario 2 — pass work in a team", () => {
  ...
  test("write_kanban feeds the cards; ctrl+k cycles list, panel, hidden, excluding the team panel", async () => {
    ...
    const { sessionId, kanban } = harness.stateStore.getState();
    expect(sessionId).not.toBeNull();
    expect(kanban.board).toEqual([{ id: "#1", content: "write the report", status: "in_progress", scope: "session", sessionId: sessionId! }]);
    await sendCmd(harness.stdin, "\x0b");
    await waitForUi(harness, (state) => state.kanban.view === "list", "kanban list view");
    await sendCmd(harness.stdin, "\x0b");
    await waitForUi(harness, (state) => state.kanban.view === "panel" && !state.teamPanelVisible, "kanban panel visible, team panel hidden");
    await sendCmd(harness.stdin, "\x1b[D");
    await sendCmd(harness.stdin, "\x0b");
    await waitForUi(harness, (state) => state.kanban.view === "hidden" && !state.teamPanelVisible, "left ignored at the panel; ctrl+k hides it");
    await sendCmd(harness.stdin, "\x1b[D");
    await waitForUi(harness, (state) => state.teamPanelVisible && state.kanban.view === "hidden", "team panel visible once kanban is hidden");
    await sendCmd(harness.stdin, "\x1b[D");
    await waitForUi(harness, (state) => !state.teamPanelVisible, "team panel hidden");
  });
