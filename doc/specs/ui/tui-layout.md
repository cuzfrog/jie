...
4. **Tool cards** (`tool-call` + matching `tool-result`) — expanded shows input, output (the reducer unwraps `{content, details, terminate}` envelopes to the string content), and a diff view when the result carries one. Collapsed, successful diff-less results fold into the **work summary**, while running calls, error results, and diff-carrying results keep one header line each (`✓`/`✗` glyph, name, formatted duration). `Ctrl+O` expands / collapses all (`state.toolCardsExpanded`).
