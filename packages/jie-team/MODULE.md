...
- Ships one blueprint, `default`: the six-role delivery pipeline of `doc/specs/jie-team/00-overview.md` (dm, researcher, architect, planner, implementer, reviewer) with constrained per-role tool sets and `notify`/artifact coordination conventions; lifecycle enforcement is platform-side future work. The two-role code-lens team lives untracked at `.jie/teams/dev` as a local test fixture.
