# ADR 18: Memory Session ID Is Per Process × Team, Supplied by the Handle
...
The v1 session-id lifecycle had three intertwined gaps:
...
- **Team-bound conversation is the user's mental model.** The per-team session id makes that literal: switching teams switches session ids. The session id is the team's "current conversation" pointer.
...
- **Multi-team per process**: in v1, per ADR 19. The `Map<team_id, session_id>` is already keyed correctly for this; the per-team model carries over without change. The `--continue` lookup, `--resume` validation, and team-swap flow are all `team_id`-aware.
...
- **Cross-team `--resume`** (deferred; see ADR 17 "Out of scope").
