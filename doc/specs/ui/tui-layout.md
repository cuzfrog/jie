...
3. **Thinking block** (one or many) — expanded renders each block as a `Thought for <duration>` label plus the markdown body recolored to `thinkingText` + italic (`durationMs` stamped by the reducer from `agent.stream.end` — `tui-state.md`; durations format as `<N>ms` under a second, `<N>s` to one decimal under a minute, `<N>m <N>s` beyond, shared `formatDuration`). Collapsed, a still-streaming block (no `durationMs` yet) renders a single italic `Thinking...` line in `thinkingText`, and completed blocks fold into the **work summary** (below). `Ctrl+T` expands / collapses all (`state.thinkingExpanded`).
