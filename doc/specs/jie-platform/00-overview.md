...
| **Tool** | A typed function with a JSON schema, available to an agent's LLM. Built-ins: `notify`, `bash`, `read_file`, `write_file`, `edit`, `kanban_write`, `web_search`, `web_fetch`, `write_artifact`, `read_artifact`. MCP-backed tools are the planned extension (not implemented today, ADR 4). |
