...
### 7.9 Decision: jie-tui runs in alt-screen and enables DECSET 1000/1006 for the chat
- v0.2 ships the whole TUI in alt-screen (`render(<App/>, { alternateScreen: true })` in `packages/jie-tui/tui.tsx`) and `chat-wheel-input` enables DECSET 1000 + 1006 while mounted. This solves the wheel-scrolls-terminal-scrollback leak: in alt-screen, the terminal's own scrollback is preserved separately and not interleaved with our frames, so even if a stray wheel event slips past Ink's input parser it scrolls an empty buffer.
- The trade-off accepted: alt-screen apps never expose terminal-native drag selection. That is a property of alt-screen mode itself (the terminal's selection layer is bound to the active screen), not specific to DECSET. To copy chat text inside the TUI we need a future in-app selection feature (claude-code's `use-selection` + `use-copy-on-select`, gated on `[?1049h`). For v0.2 the user accepts that copying chat text is via the editor's existing selection (keyboard) or via piping `jie -p` output.
- DECSET 1002/1003 are deliberately not enabled: those modes hijack every motion event and break even in-app selection that a future feature might add. Mode 1000 + SGR 1006 is the minimum needed for wheel.
- The flow is still single-source-of-truth in `parseKeypress`: buttons 64/65 → `wheelup`/`wheeldown`, buttons & 0x40 === 0 and X10 legacy → `'mouse'` (absorbed). `nonAlphanumericKeys` clears `input` so the editor never sees raw mouse bytes.
...
- jie-tui runs in alt-screen and enables DECSET 1000/1006 from inside the chat pane ([§7.9](#79-decision-jie-tui-runs-in-alt-screen-and-enables-decset-10001006-for-the-chat)): this fixes the wheel-scrolls-terminal-scrollback leak. The trade-off is that alt-screen apps never expose terminal-native drag selection, so chat-text copy inside the TUI is deferred to a future in-app selection feature.
