...
**Body's subscription callback receives the envelope.** When a body receives a message on a subscribed subject, the bus invokes the callback with `(subject, envelope)`. The body reads `envelope.payload.prompt` for user prompts (the `team.{teamId}.agent.{agentKey}.prompt` envelope payload is `{ teamId, agentKey, prompt }`), and `envelope.payload` (a string) for `notify`-sourced events. The synthetic `user`-message formats are documented in `06-agent-model.md` "Prompt Ingress & Queuing".
