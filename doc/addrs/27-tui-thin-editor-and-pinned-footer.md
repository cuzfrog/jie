...
- `packages/jie-tui/components/layout.test.tsx` (+2 tests), `packages/jie-tui/components/panel/editor.test.tsx` (+3 layout/visual tests + 6 cursor tests) — both files are no-new-exports but the added tests are pure additions to the file's `describe` block; no new exports cross the file's seam.
