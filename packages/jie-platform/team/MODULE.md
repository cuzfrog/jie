...
- On a fresh load the `system.team.loaded` payload carries the restored per-agent history (`TeamInfo.history`, non-empty `messages`); the returned identity carries empty per-agent `history`. The TUI hydrates its conversation display from the event and treats the identity as focus/agent reconciliation only (see `08-memory.md` Load ordering).
