...
describe("Footer", () => {
  ...
  test("renders two lines: identity with cwd/branch left and team:agent right", () => {
    ...
    const lines = new Footer(stateStore, tokenUsage, modelSegment, queueIndicator).render(80);
  ...
  test("keeps both lines while the kanban list view is shown", () => {
    ...
    expect(new Footer(stateStore, tokenUsage, modelSegment, queueIndicator).render(80).length).toBe(2);
  });
  ...
  test("line two shows the /help hint between the context metrics and the model", () => {
    ...
    const plain = stripAnsi(new Footer(stateStore, tokenUsage, modelSegment, queueIndicator).render(80)[1]);
  ...
  test("every line fits the given width", () => {
    stateStore.getState.mockReturnValue(seededState(true));
    const lines = new Footer(stateStore, tokenUsage, modelSegment, queueIndicator).render(60);
  ...
  test("never renders a line wider than the given width with over-long identity (doRender guard)", () => {
    ...
    const footer = new Footer(stateStore, tokenUsage, modelSegment, queueIndicator);
