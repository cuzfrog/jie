...
**The platform ships a hardcoded built-in minimal team as a last-resort fallback.** It is defined in `packages/jie-platform/team/built-in/minimal-team.ts` — a TypeScript constant, not a `.md` file. It is used only when no user team is selected (no `--team` flag, no `defaultTeam` in settings, no user team manifests at the standard paths). It is **not** imported from `@cuzfrog/jie-team`; it lives in the platform code. The platform is still agnostic of `jie-team` as a package — no `import`, no `dependency`, no `postinstall` hook. The `jie-team` package may ship its own minimal team `.md` files which, once copied to `~/.jie/teams/minimal/` or `.jie/teams/minimal/`, override the platform's built-in. The built-in is the v1 "always has something to run" guarantee. See `10-configuration.md` "Team Selection" and `minimal-team.md`.
...
- **The built-in fallback is a platform constant, not a `.md` file.** The platform's hardcoded minimal team is a TypeScript constant. The `jie-team` package's `minimal/` `.md` files are an optional user-installable override. The two are independent: the platform does not import from `jie-team`, and the `jie-team` package does not import from the platform at runtime.
...
- `10-configuration.md` "Team Selection" describes the platform's resolution chain (`--team`, `defaultTeam`, first-available user team, built-in minimal) and the stale-recovery rule. The platform never fails on "no teams available" — the built-in is the last-resort fallback. It does not mention `jie team install` or `postinstall`.
- `12-installation.md` "First-Run Credentials and Model" notes that the platform always has a runnable team (the built-in minimal) and `jie login` + `jie model` are the only setup required. "Installing a User Team" describes the manual copy pattern: place `TEAM.md` and one `.md` per role at the standard paths, then run `jie team <id>` (or `/team <id>` in the TUI) to set `defaultTeam`.
