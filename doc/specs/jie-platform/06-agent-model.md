...
- The platform enforces **workspace-root containment only**. It does **not** check module boundaries, no-new-exports rules, or any other team-defined constraint. Those checks are the team layer's responsibility — see "Boundary Enforcement" below.
...
`write_file` enforces only **workspace-root containment** (the `path_escape` rule above). It does **not** enforce module boundaries (no-new-exports rules, descriptor checks). The two enforcement layers are distinct:

| Layer | What it enforces | When it runs | v1 status |
|---|---|---|---|
| Platform `write_file` | "Inside the workspace root" | At the tool call | v1 (this spec) |
| Team descriptor / no-new-exports rule | "Inside the allowed module boundary" | At the role's system prompt or via a wrapper tool that the team defines | Day 2+ (when teams need module-boundary enforcement) |
...
**Consequence:** in v1, an agent with `write_file` in its tool list can write any file inside the workspace root, including files inside a no-new-exports module. The team layer is responsible for preventing that, not the platform. This is an explicit Day-1 commitment: boundary enforcement is a team-layer contract.
...
When a message arrives on Jie's EventBus (via `team.{teamId}.agent.{agentKey}.prompt` or a `custom.{teamId}.{topic}` subscription), the body formats it as a synthetic `user` `AgentMessage` and ingests via `agent.prompt()`. The bus invokes the subscription callback with `(subject, envelope)`; the body reads `envelope.payload` for the prompt text. The format is keyed on the topic shape: a `team.…agent.…prompt` event (TUI/CLI-published) carries no source, so the body formats it as `[user]: {prompt}`; a `custom.…` event is `notify`-sourced (an agent publishes it via `Events.custom`), so the body formats it as `[{source_agent_key} on '{topic}']: {prompt}` where `topic` is the unscoped name from the publisher's `notify` call. TUI-published direct-addressed user prompts (per `ui/tui-overview.md` "Role") follow the same `team.…prompt` format.

| Source | `envelope.payload` shape | Synthetic `user` message `content` |
|---|---|---|
| `team.…agent.…prompt` (TUI / `-p` mode / direct addressing) | `{ teamId, agentKey, prompt }` | ``[user]: {prompt}`` |
| `custom.{team_id}.{topic}` (from `notify` between bodies) | `string` (the prompt text) | ``[{source_agent_key} on '{topic}']: {prompt}`` |

Both formats are plain text — v1 has no image / multimodal content for synthetic user messages. `content` is always a single `string`. The body converts the prompt text verbatim; no escaping, no parsing of inner newlines, no parsing of the recipient's intent. The receiving LLM extracts identifiers, topic names, and structure from the text as part of its reasoning.
...
**Queue observability.** The body publishes `agent.prompt.queue.update` on every enqueue and every dequeue, mirroring pi's `queue_update` event. The payload is `{ prompts: string[] }` — the current snapshot of the queue. Each element of `prompts` is the synthetic `user`-message format the body would feed to `agent.prompt()`: ``[user]: {prompt}`` for `leader.prompt` and ``[{source_agent_key} on '{topic}']: {prompt}`` for `notify`-sourced prompts (see "Prompt Ingress & Queuing" above). The TUI subscribes to this event to render the queued-prompt indicator (e.g., "3 prompts queued") and a peek of the contents. Without this event, the TUI would have to derive queue state from "leader is not idle", which is brittle when multiple agents are active.
