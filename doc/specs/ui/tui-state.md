...
- `Actions.commitKanbanEdit(cardId)` — set `state.kanbanEdit = cardId`; the editor takes over for the card's content (`tui-layout.md`, Editor). Wired to `Ctrl+E` while the panel view is shown and a card is focused (`Enter` stays the prompt submit; `tui-kanban-panel.md`).
