...
- Architect - on `task.researched`, the sole role that authors module contracts and inspects codebase structure beyond descriptors (via code-lens); updates `CONTEXT.md` files (only the Architect may author descriptors — see "Module descriptor and the sealed boundary") and emits `task.designed`.
