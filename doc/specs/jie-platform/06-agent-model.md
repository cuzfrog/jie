...
The body owns compaction triggering; pi-agent's own path (`CompactionSettings` inside `transformContext`) never runs — the body passes an identity `transformContext`. The `agentBodyFactory` constructs one `Compactor` per body from the shared `memoryManager` and the body's identity params. The contract is `compact(input): Promise<CompactionResult | null>` — `null` means nothing was compacted (below threshold, nothing summarizable, or no model assigned).
