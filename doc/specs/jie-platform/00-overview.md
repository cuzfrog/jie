...
| **Session** | A `session_id` (ULID) partitions conversation history per process × team. `createJiePlatform` validates `JiePlatformOptions.resumeSessionId` (`--resume`) via `hasSession`, else mints a fresh id, and records it in a private `Map<team_id, session_id>` — in-memory only, lost on process exit; `memory_turns` rows persist indefinitely, so prior sessions remain resumable. ADR 17. |
