...
- **Parsing**: strip code fences, `JSON.parse`; an atom with an invalid `type` is dropped; `priority` clamps to 0–100; `instruction` forces 100. A parse failure drops the batch (WARN), never a partial write.
