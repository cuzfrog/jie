...
- `Actions.clearTuiState()` — clear `agents`, `sessionName`, `leaderAgentId`, `focusedAgentId`, `teamCursorAgentId`, `interruptedAgentId`, `infoEntries`, and `errorBanner` / `transientMessage`, and reset `nextEntrySeq` to 0. `teamPanelVisible` survives like the other view toggles. Memory rows on disk are untouched. Used by the `/clear` slash command.
