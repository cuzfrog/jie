...
4. **Tool cards** (`tool-call` + matching `tool-result`) — one header line: the `✓` glyph in `success` (`✗` in `error` for failures), then name and formatted duration in `toolTitle` (`error` for failures). A result carrying a diff (`details.kind === "diff"`, from `edit` and `write_file`) always renders the diff block below the header — collapsed or expanded: `DiffView` recolors the unified diff (add `success`, del `error`, context `text`, `@@` meta `muted`) with a line-number gutter — old-file numbers on `-` lines, new-file numbers on `+` and context lines, right-aligned to the widest number, `muted`; a header-less diff renders unnumbered. Expanded additionally shows input, output (the reducer unwraps `{content, details, terminate}` envelopes to the string content), and the error text. Collapsed, successful diff-less results fold into the **work summary**; running calls, error results, and diff-carrying results stay individual. `Ctrl+O` expands / collapses all (`state.toolCardsExpanded`).
