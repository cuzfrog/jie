---
name: tui-fix
description: Fix tui problems, enhance implementation and tests.
---

## Goal
Fix tui problems, enhance implementation and tests.

## What you do
- Adhere to @CLAUDE.md.
- Analyze the code and read documents under `doc/specs/ui/`. Ensure references and documents are consistent.
- Discuss with the me upon blocking or important issues. Offer your solutions. Be honest, if the my idea does not make sense, push back. Ask me questions if you need to.
- I may have modified the code, follow my intentions and complete the implementation, do not revert my changes or overhual the shape without raising to me.
- If you think better to add new exports to no-new-exports files, type out the code to be exported for me to review, if I agree, I will manually add the exports.
- Once a problem is found, update/add tests coverage first, run the test, ensure the test will fail (reproduce the problem) before implementation the fix. Unit tests should be collocated in companion test files.
- Any logic change should be reflected by unit test accordingly.

## Issues or instructions by user:
$ARGUMENTS
