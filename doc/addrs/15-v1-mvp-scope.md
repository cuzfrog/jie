...
- The platform's `startJie` returns a `JieHandle` whose `loadTeam` method's user-team lookup uses the standard-path resolution per `10-configuration.md`. With no user teams installed and no dev team shipped, `loadMinimalTeam()` is the only team the v1 MVP can resolve to (after the user runs `jie login` and `jie model`).
