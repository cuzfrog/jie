...
describe("render — chat pane order (T2 fixtures)", () => {
  ...
  test("tool cards render inline (T2 worker path)", () => {
    ...
    state = reduce(state, Actions.receiveEvent(Events.agentToolCall(AGENT_SENDER, "c1", "read_file", "input.txt", false)));
    state = reduce(state, Actions.receiveEvent(Events.agentToolResult(AGENT_SENDER, "c1", "read_file", "file content", false, 12, null)));
    const frame = render(state, { cols: 80, rows: 30 }, NOW);
