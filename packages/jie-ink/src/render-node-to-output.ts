...
const renderNodeToOutput = (
	...
	if (yogaNode) {
		...
		if (node.nodeName === 'ink-box') {
			...
			const overflowY =
				node.style.overflowY === 'scrollBottom' ||
				node.style.overflow === 'scrollBottom'
					? 'scrollBottom'
			...
			const clipVertically = overflowY !== 'visible';

			if (overflowY === 'scrollBottom') {
				...
				for (const child of childNodes) {
					child.yogaNode?.setFlexShrink(0);
				}
				...
				let contentBottom = 0;
				for (const child of childNodes) {
					const cy = child.yogaNode?.getComputedTop() ?? 0;
					const ch = child.yogaNode?.getComputedHeight() ?? 0;
					const cb = cy + ch;
					if (cb > contentBottom) {
						contentBottom = cb;
					}
				...
				if (process.env['JIE_INK_DEBUG_SCROLL'] === '1') {
					process.stderr.write(
						'[scrollBottom] boxInner=' +
							String(boxInnerHeight) +
							' contentBottom=' +
				...
				if (contentBottom > boxInnerHeight) {
					childYOffset = -(contentBottom - boxInnerHeight);
				}
			}
