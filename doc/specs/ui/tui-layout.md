...
The **queued prompts** section sits between the status line and the editor (`components/queued-prompts.ts`): one line per entry of the focused agent's `queue` — the raw user text carried by the `agent.prompt.queue.update` snapshot (`tui-state.md`) — prefixed `Queued: ` in `muted`, each truncated to the width. Empty when the queue is empty or no agent is focused. It complements the footer line-2 queue segment (count + next-prompt preview) by listing every pending prompt verbatim, pi-style, right above the input.
