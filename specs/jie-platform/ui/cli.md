...
8. Import `jie-tui`, pass `EventBus` + `ArtifactStore` + `roles` (the startup team's role stems, sorted alphabetically — sourced from `handle.rolesFor(startupTeamId)`), start TUI. (Stub in v1 per ADR 17; throws on call.)
