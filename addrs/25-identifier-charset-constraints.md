# ADR 25: Identifier Charset Constraints — Hard Fail at Load

## Status

Accepted 2026-06-14. Closes Pass 10 items B1 and B2 (commit-fence: `review-tracker.md` Pass 10).
...
| Identifier | Documented charset | Documented since | Enforced at use? |
|---|---|---|---|
| `team_id` (the team directory name) | `[A-Za-z0-9_-]{1,32}` | ADR 19 (memory team-scoping, 2026-06-13) | No — `defaultTeam` and `--team <id>` are validated; the team-blueprint loader was not. |
| Agent role (filename stem) | "(no constraint)" | Pass 9 (2026-06-14) — explicitly noted as "no constraint" | No — the loader used `path.parse(file).name` verbatim. |
...
The role stem has been explicitly unconstrained since ADR 18 (filename-stem-canonical, 2026-06-12). The "no constraint" note in the limits table acknowledged the gap but did not resolve it.

The Pass 10 review surfaced three real consequences of the un-enforced state:
...
- Closes Pass 10 items B1 and B2 (`review-tracker.md`).
- Modifies: `06-agent-model.md` (parse-errors table), `10-configuration.md` (Platform Limits table), `packages/jie-platform/team/loader.ts` (the actual enforcement — implementation).
- Depends on: ADR 18 (filename-stem-canonical), ADR 19 (memory team-scoping, which introduced the `team_id` charset).
- Closes the Pass 10 review on the identifier-charset question. No follow-up ADR needed.
