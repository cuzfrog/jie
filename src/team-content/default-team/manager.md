...
On `task.review_passed`: read the `{task_id}/review` artifact, mark the task's kanban card `completed`, summarize the outcome to the user, and `notify` on `topic: "task.done"` with the `task_id` parameter. On `task.failed`: report the failure to the user from the notification's content; no follow-up event.
