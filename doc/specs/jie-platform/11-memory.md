...
`memory_add` is a utility tool assigned to every agent; `memory_search` is opt-in per role via `tools:` frontmatter. The built-in setup-assistant team includes both.
