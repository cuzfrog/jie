...
- `tui-kanban-panel.md` — the toggleable bottom kanban panel, the focused agent's card board.
...
- **Out-of-band oblivious, except compaction.** Memory loads are not published on the EventBus and so the TUI does not display them. Compaction is the exception: a successful rewrite publishes `agent.compacted`, which the TUI renders as a marker entry in the chat column (`tui-state.md`).
...
- **`system.team.loaded` is the boot anchor.** The CLI executes the team load after the TUI resolved from `bootTui(...).cradle.tui` has subscribed, so by the time `tui.start()` builds the component tree, `state.agents` is already populated. The TUI does not take a `roles` bootstrap parameter.
...
`bootTui(options, deps)` is called by the CLI's `jie` entry after `bootPlatform` has run and returns an `AwilixContainer<TuiCradle>`; the `Tui` handle is `cradle.tui`. The TUI does not load teams on its own. It validates an interactive terminal (`process.stdin.isTTY`, unless `deps.stdin` is supplied) and a UTF-8 locale; `start()` requires at least 60 columns and resolves on quit. `TuiDeps` is `{ platform, gitBranch, gitDirty }` plus optional `stdin`/`stdout`/`stderr` streams. The CLI passes `platform` and a git snapshot (read from the platform container's `gitService.getSnapshot()`) for the footer's branch/dirty display; the streams are injection points used by tests (`start()` uses pi's `ProcessTerminal` over `process.stdin/stdout` when `deps.stdin` is absent, and a stream terminal over the injected streams otherwise). `options.rows` is vestigial — pi-tui reads the terminal dimensions itself. The `JiePlatform` facade exposes `subscribe(topic, cb)`, `prompt(teamId, agentKey, text)` (the sole prompt ingress), `interrupt(teamId, agentKey)`, and `execute(command)` — a single command channel carrying named operations. The slash-command handler (`command-handler.ts`) is the only `execute` caller.
...
`jie [--team <id>] [--resume <id>] [--in-memory]` opens the TUI. The TUI and `-p` share the platform boot (`bootPlatform`); the only difference is the final render surface. The TUI does not accept `-p`, `--json`, `--timeout`, or `--api-key`.
