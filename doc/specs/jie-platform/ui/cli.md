...
8. Import `jie-tui`, pass `EventBus`, start TUI. (Stub in v1 per ADR 15; throws on call.) The TUI's role stems and per-agent roster come from the `team.loaded` event on the bus (per ADR 25).
