...
const expectations: Expectation[] = [
  turn(DEV, 0, toolCall("d1", "artifact", JSON.stringify({ op: "write", key: "greet-task/consultation", content: CONSULTATION }))),
  ...
  turn(DEV, 7, toolCall("d8", "artifact", JSON.stringify({ op: "write", key: "greet-task/review_request", content: REVIEW_REQUEST }))),
  ...
  turn(ARCH, 0, toolCall("a1", "artifact", JSON.stringify({ op: "read", key: "greet-task/consultation" }))),
  turn(ARCH, 1, toolCall("a2", "artifact", JSON.stringify({ op: "write", key: "greet-task/architect_answer", content: ARCH_ANSWER }))),
  ...
  turn(PEER, 0, toolCall("p1", "artifact", JSON.stringify({ op: "read", key: "greet-task/review_request" }))),
  turn(PEER, 1, toolCall("p2", "bash", JSON.stringify({ command: "bun test greet.test.ts" }))),
  turn(PEER, 2, toolCall("p3", "artifact", JSON.stringify({ op: "write", key: "greet-task/review", content: REVIEW }))),
