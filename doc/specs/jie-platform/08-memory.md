...
On agent start, the body's `session_id` is supplied by `createJiePlatform` (per ADR 13 and ADR 18). The platform owns the session map (a closure field); the body is a runtime that uses the value it's given.
...
The platform's private `Map<team_id, session_id>` is in-memory only; it is lost on process exit. A fresh process run starts with an empty map. Each new team seen by the platform mints a fresh session id and records the mapping; team swap back to a previously-active team (Day 2+) reuses the recorded session id. Memory is per-team at the session level, per-agent at the row level.
...
**Team scoping.** `team_id` is the namespace for `memory_turns`. Two teams that both contain a role named `general` (and thus both have `agent_key = general-1`) live in disjoint row sets because their `team_id` differs. Users can name roles freely across teams — including reusing names — without collision. The platform's private in-memory `Map<team_id, session_id>` (per ADR 18) is keyed by `team_id` alone — the per-team model means all agents in a team share one session id, so per-`agent_key` disambiguation is not needed. Per-team `--continue` lookups filter on `team_id`; the `idx_memory_turns_team_session_created` index on `(team_id, session_id, created_at)` makes those lookups efficient.
...
- **`transformContext` wrapper**: The body passes a wrapped `transformContext` to pi-agent via `AgentOptions`. In v1, the wrapper is the identity function (compaction is deferred to Day 2; see below). The Day 2+ design is: the wrapper calls the inner `transformContext` (pi-agent's default or the Day 2 compaction implementation), diffs the input and output arrays, and for each `CompactionSummaryMessage` that appears in the output but not the input, computes the seq range and calls `memory.compact(range, summary, agent_key, session_id, team_id)`. The wrapper returns the new array unchanged. Compaction is trigger-agnostic — whether the trigger is the user's `/compact` slash command, pi-agent's auto-threshold, or a Day 2+ team hook, all paths funnel through `transformContext` and the wrapper persists. **(v1: wrapper is identity — diff+compact is Day 2.)**
...
- **Restore**: On agent start, the body uses the `session_id` supplied by the platform (per ADR 18 and ADR 20). The body's `start()` runs the four-step restore-and-start sequence documented in `06-agent-model.md` "AgentBody" `start()`:
...
**v1 deferral.** The `MemoryManager.compact()` API exists at the storage level and is unit-tested, but the body's `transformContext` wrapper is currently an identity function in v1. There is no path that calls `MemoryManager.compact()` in v1: the diff-and-persist hook is deferred to Day 2. A partial implementation in v1 would be worse than a clean deferral — the storage layer is ready (and the API is tested), but the body never triggers it. Day 2 will land the diffing wrapper and the trigger wiring together.
