...
- `parseSkill` validates a single SKILL.md's content (name charset/length, frontmatter, name/directory match, description) with no I/O and captures the prose body (trimmed) for invocation expansion; an invalid skill is reported via the `diagnostic` field, never thrown.
