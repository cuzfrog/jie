...
|| **DynamicHint** | Context-dependent hint segment in the TUI footer stats line: applicable hints joined by ` \| `, falling back to the `/help` hint; when terminal width is insufficient a single hint is chosen at random. | `ui/` (TUI footer) |
...
|| **Tool** | Typed function with a JSON schema. Built-ins: `bash`, `read_file`, `write_file`, `edit_file`, `ls`, `find_file`, `grep_file`, `artifact`, `write_kanban`, `update_kanban`, `memory`, `notify`, `call_agent`, `ask_user_questions`, `web_search`, `web_fetch`, plus unimplemented MCP tools (ADR 4). | `06-agent-model.md` |
